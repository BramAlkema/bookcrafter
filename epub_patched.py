...
class EpubHtml(EpubItem):
    ...
    def get_content(self, default=None):
        ...
        # Calculate relative paths for links based on document location
        doc_dir = zip_path.dirname(self.file_name)
        ...
        for lnk in self.links:
            if lnk.get("type") == "text/javascript":
                ...
                # Fix: Calculate relative path from document to linked resource
                fixed_lnk = lnk.copy()
                if "href" in fixed_lnk and doc_dir:
                    fixed_lnk["href"] = zip_path.relpath(lnk["href"], doc_dir)
                _lnk = etree.SubElement(_head, "link", fixed_lnk)
